...
For the priority variables, all data is accessible `live' on the `canari' Group Workspace on JASMIN at `/gws/ssde/j25b/canari/large-ensemble/priority-variables`. In addition, the PVs are pre-separated into single variable files, for example:
...
Because of the availability of the PVs on disk, the process for obtaining the CFA files representing each realm is greatly simplified and is performed by a single Python script, \texttt{priority-variables.py}. This script reads in all the data for each member of the historical and SSP3-7.0 ensembles and  


\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/ATM}
    \caption{Example surface temperature after timestep (in Kelvin) from ensemble member 1 of the historical ensemble.}
    \label{fig:ATM}
\end{figure} 

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/OCN}
    \caption{Example sea surface temperature (in Celsius) from ensemble member 1 of the historical ensemble.}
    \label{fig:OCN}
\end{figure} 

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/CICE}
    \caption{Example sea ice area (aggregate) from ensemble member 1 of the historical ensemble. The quantity is dimensionless and the contour lines shown are at 0 (no ice), 0.2, 0.4, 0.6, 0.8 and 1 (full coverage).}
    \label{fig:CICE}
\end{figure} 
